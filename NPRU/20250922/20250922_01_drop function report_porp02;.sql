drop function report_porp02;

CREATE OR REPLACE FUNCTION erp.report_porp02(p_lin_id character varying, p_company_code character varying, p_doc_type character varying, p_s_div character varying, p_e_div character varying, p_s_po_no character varying, p_e_po_no character varying, p_s_po_date character varying, p_e_po_date character varying, p_product_type character varying, p_status character varying, p_user_id integer)
...
BEGIN
    ...
    SELECT
        ...
        case when a.ref_doc_type = 'CO' 
    		...
    		end::character varying::character varying AS ref_doc_no,
        (
            CASE 
        ...
        case when a.ref_doc_type = 'CO' 
        		then pml2.method_name
        		else pml.method_name
    		end::character varying::character varying AS method_name,
	...
	from PO_PUR_ORD_MASTER a
		...
		left join db_list_value_lang dlvl2 on dlvl2.group_code = 'PoStatusPO' 
	...
	left join po_pur_ord_set d on d.pur_ord_detail_id = b.pur_ord_detail_id 
		...
		and d.po_no = b.po_no 
		and d.seq = b.seq
	left join db_list_value_lang p_List_Value_Status on p_List_Value_Status.group_code = 'PoStatusPO' 
	...
	left join PO_PUR_ORD_MASTER a2 on a2.company_code = a.company_code 
		and a2.doc_type  = a.ref_doc_type  
		and a2.po_no = a.ref_doc_no 
		left join PO_PR_MASTER c2 on c2.company_code = a2.company_code 
				and c2.doc_type = a2.ref_doc_type  
				and c2.pr_no = a2.ref_doc_no
				left join po_method_lang pml2 on c2.pr_method_id = pml2.method_id  
						and lower(pml2.language_code) = lower(p_lin_id)
