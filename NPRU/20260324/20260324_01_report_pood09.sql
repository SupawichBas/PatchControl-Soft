...
CREATE OR REPLACE FUNCTION erp.report_pood09(p_lin_id character varying, p_receive_master_id integer)
 RETURNS TABLE(receive_master_id integer, doc_type character varying, receive_date date, doc_receive_date character varying, pr_no character varying, pr_date date, pr_doc_date character varying, ref_div_code character varying, ref_div_name character varying, ap_member_id integer, ap_member_name character varying, pr_method_id integer, method_name character varying, subject character varying, total_amt numeric, baht_text character varying, total_amt_l numeric, total_fine_amt numeric, recipient_head_image character varying, head_emp_name character varying, ref_doc_no character varying, period_receive_detail character varying, detail1 character varying)
 LANGUAGE plpgsql
AS $function$
BEGIN
    RETURN QUERY
...
--			,dol.organization_name::character varying as ref_div_name
			...
			,coalesce((select sum(ppopm.period_amt) from po_pur_ord_period_master ppopm 
						where ppopm.company_code = a.company_code 
						and ppopm.doc_type = a.ref_doc_type 
						and ppopm.po_no = a.ref_doc_no),coalesce(a.total_amt,0) - coalesce (a.total_fine_amt,0))::numeric as total_amt
			,erp.fn_baht_text(coalesce((select sum(ppopm.period_amt) from po_pur_ord_period_master ppopm 
						where ppopm.company_code = a.company_code 
						and ppopm.doc_type = a.ref_doc_type 
						and ppopm.po_no = a.ref_doc_no),coalesce(a.total_amt,0) - coalesce (a.total_fine_amt,0)))::character varying AS baht_text
			,coalesce(a.total_amt,0) - coalesce (a.total_fine_amt,0)::numeric as total_amt_l
			...
			TO_CHAR(coalesce(a.total_amt,0) - coalesce (a.total_fine_amt,0)::numeric, 'FM9,999,999.00'),' ',
			erp.get_report_label_name('POOD09', 'THB', p_lin_id)
			,' (',erp.fn_baht_text(coalesce(coalesce(a.total_amt,0) - coalesce (a.total_fine_amt,0),0)),')')::character varying AS detail1
	from po_receive_master a 
	left join db_list_value_lang receivedate  on receivedate.group_code = 'Month' 
			and receivedate.value = TO_CHAR(a.deliver_date , 'FMMM')
