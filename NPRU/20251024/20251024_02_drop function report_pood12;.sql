...
FROM ( select a.pr_master_id::integer AS pr_master_id,
        ...
        case when a.report_date is not null then 
					case p_lin_id when 'TH' then 
							concat(to_char(a.report_date, 'FMDD') ,' ' ,
							Monthlang.value_text ,' ' ,
							to_char(extract(year from a.report_date) +543 ,'FM9999'))
						else
							concat(to_char(a.report_date, 'FMDD') ,' ' ,
							Monthlang.value_text ,' ' ,
							to_char(extract(year from a.report_date) ,'FM9999'))
						end
					end::character varying as report_date,
		...
		concat('<b>',get_report_label_name('POOD12', 'header_1', p_lin_id),'</b> ',
		case when a.report_std_price is not null 
			then concat(to_char(COALESCE(a.report_std_price, 0)::numeric, 'FM999,999,999,990.00')
					,' ',get_report_label_name('POOD12', 'header_1_1', p_lin_id)
					,' (',bgst_get_baht_text(a.report_std_price),') ')
			else ' ' end
		)::character varying AS header_1,
		concat(get_report_label_name('POOD12', 'header_2', p_lin_id))::character varying AS header_2,
		concat(get_report_label_name('POOD12', 'header_2_1', p_lin_id))::character varying AS header_2_1,
		concat(get_report_label_name('POOD12', 'header_3', p_lin_id))::character varying AS header_3,
		concat(get_report_label_name('POOD12', 'header_3_1', p_lin_id))::character varying AS header_3_1
	from po_pr_master a
		left join db_list_value_lang Monthlang  on Monthlang.group_code = 'Month' 
				and Monthlang.value = TO_CHAR(a.report_date, 'FMMM')
