DROP FUNCTION erp.report_pood01(varchar, int4);

CREATE OR REPLACE FUNCTION erp.report_pood01(p_lin_id character varying, p_pr_master_id integer)
 RETURNS TABLE(pr_master_id integer, company_code character varying, pr_no character varying, doc_type character varying, div_code character varying, div_code_display character varying, div_name character varying, ref_no character varying, pr_date date, pr_req_date character varying, subject character varying, attn_to character varying, purpose character varying, bg_div_code character varying, bg_div character varying, budget_year integer, fund_code_name character varying, fsource_code_name character varying, prod_code_name character varying, bg_sec_code_name character varying, atv_code_name character varying, maingfmis character varying, gfmis character varying, product_type_name character varying, complete_day integer, master_remark character varying, pr_detail_id integer, pr_set_id integer, forward_id character varying, bf_amt numeric, total_amt numeric, bel_amt numeric, recipient_image character varying, recipient_emp_name character varying, recipient_posi character varying, recipient_head_image character varying, head_emp_name character varying, head_posi character varying, pre_approve_level1_head_image character varying, pre_approve_level1_emp_name character varying, pre_approve_posi_level1 character varying, recipient_sign_image character varying, sign_name character varying, sign_posi character varying, personal_head_image character varying, personal_emp_name character varying, personal_posi character varying, pr_set_running_no integer, status_show_image boolean, status_show_head_emp_image boolean, status_show_pre_approve_id_level1_image boolean, status_show_sign_image boolean, approve_date_head_emp_id character varying, approve_date_pre_approve_id_level1 character varying, approve_date_sign_id character varying, atv_id integer, pr_budget_id integer, pr_detail_running_no integer, group_atv_id integer)
 LANGUAGE plpgsql
AS $function$
BEGIN
    RETURN QUERY
SELECT var.*,
        DENSE_RANK() OVER (ORDER BY var.atv_id,var.pr_detail_id)::integer AS pr_detail_running_no,
        DENSE_RANK() OVER (ORDER BY var.atv_id)::integer AS group_atv_id
FROM ( select a.pr_master_id::integer AS pr_master_id,
        ...
        concat(ol.organization_name,' ', mainlang.organization_name)::character varying AS div_name,
        ...
        case when deh.personal_id is not null and deh.signature_image_id is not null
			...
			else null end::character varying as recipient_image,
        deh.emp_name::character varying AS recipient_emp_name,
        a.head_posi::character varying AS recipient_posi,
        --
        case when deh.personal_id is not null and deh.signature_image_id is not null
			then concat((select sp.parameter_value  from su_parameter sp 
							where sp.parameter_group_code = 'ContentPath'
        ...
        case when depersonal.personal_id is not null and depersonal.signature_image_id is not null
			...
			else null end::character varying as personal_head_image,
        depersonal.emp_name::character varying AS personal_emp_name,
        ''::character varying AS personal_posi,
        --
		...
		else false end::bool as status_show_head_emp_image,
        case when po_get_show_image(a.company_code,a.doc_type,a.pr_no,a.pre_approve_id_level1 ) = 'Y' then true
		else false end::bool as status_show_pre_approve_id_level1_image,
		...
		erp.po_get_fromat_approve_date((select pwf.approve_date from po_work_flow pwf 
											...
											and pwf.doc_type = a.doc_type 
											and pwf.doc_no = a.pr_no 
											and pwf.approve_by_id = a.sign_id)::date)::character varying AS approve_date_sign_id,
	...
	from po_pr_master a
		...
		left join db_organization o on a.company_code = o.company_code 
				...
				left join db_organization_lang mainlang on mainlang.company_code = o.company_code 
			            and mainlang.organization_code = o.main_organization_code 
			            and lower(mainlang.language_code) = lower(p_lin_id)
...
--	order by a.pr_no ,b.seq ,c.set_seq
	...
	order by var.pr_no ,var.atv_id,var.pr_detail_id;
END;
$function$
;
