...
BEGIN
    ...
    SELECT 
        ...
        concat('<b>ส่วนราชการ</b> ',ol.organization_name,' ', mainlang.organization_name)::character varying AS subject_1,
        ...
        REGEXP_REPLACE(a.content1, '^\s+|\s+$', '', 'g')::character varying AS detail_section_1,
        concat('๒. ขอบเขตของงาน')::character varying AS section_2,
        REGEXP_REPLACE(a.content2, '^\s+|\s+$', '', 'g')::character varying AS detail_section_2,
        concat('๓. ราคากลางของพัสดุ')::character varying AS section_3,
        REGEXP_REPLACE(a.content3, '^\s+|\s+$', '', 'g')::character varying AS detail_section_3,
        concat('๔. วงเงินงบประมาณ')::character varying AS section_4,
        REGEXP_REPLACE(a.content4, '^\s+|\s+$', '', 'g')::character varying AS detail_section_4,
        concat('๕. กำหนดระยะเวลาดำเนินงาน/ส่งมอบงาน')::character varying AS section_5,
        REGEXP_REPLACE(a.content5, '^\s+|\s+$', '', 'g')::character varying AS detail_section_5,
        concat('๖. วิธีที่จะจ้างและเหตุผลที่ต้องจ้างโดยวิธีนั้น')::character varying AS section_6,
        REGEXP_REPLACE(a.content6, '^\s+|\s+$', '', 'g')::character varying AS detail_section_6,
        concat('๗. หลักเกณฑ์การพิจารณา')::character varying AS section_7,
        REGEXP_REPLACE(a.content7, '^\s+|\s+$', '', 'g')::character varying AS detail_section_7,
		concat('8. ข้อเสนออื่น ๆ')::character varying AS section_8,
		REGEXP_REPLACE(a.content8, '^\s+|\s+$', '', 'g')::character varying AS detail_section_8,
        concat('จึงเรียนมาเพื่อโปรดพิจารณา')::character varying AS therefore,
        REGEXP_REPLACE(a.content9, '^\s+|\s+$', '', 'g')::character varying AS therefore_1,
		CASE 
