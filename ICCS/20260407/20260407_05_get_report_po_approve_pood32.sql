DROP FUNCTION erp.get_report_po_approve_pood32(varchar, varchar, varchar, varchar);
...
CREATE OR REPLACE FUNCTION erp.get_report_po_approve_pood32(p_lin_id character varying, p_company_code character varying, p_doc_type character varying, p_doc_no character varying)
 RETURNS TABLE(work_flow_id integer, label_sign character varying, signature_image character varying, emp_name character varying, position_approve_desc character varying, approve_date character varying, approve_date_format character varying, show_image boolean)
 LANGUAGE plpgsql
AS $function$
BEGIN
    RETURN QUERY
