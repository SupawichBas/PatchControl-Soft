DROP FUNCTION erp.get_report_po_approve(varchar, varchar, varchar, varchar);

CREATE OR REPLACE FUNCTION erp.get_report_po_approve(p_lin_id character varying, p_company_code character varying, p_doc_type character varying, p_doc_no character varying)
 RETURNS TABLE(work_flow_id integer, label_sign character varying, signature_image character varying, emp_name character varying, position_approve_desc character varying, approve_date character varying, show_image boolean)
 LANGUAGE plpgsql
AS $function$
BEGIN
    RETURN QUERY
select pwf.work_flow_id::integer as work_flow_id,
    ...
    case when pwf.acting_for is not null then concat(concat(pwf.position_approve_desc),' ',erp.get_program_label_name('ALL', 'PoWorkFlow.ActingFor', p_lin_id),' ', pwf.acting_for)
        else concat(pwf.position_approve_desc) end::character varying AS position_approve_desc,
    erp.po_get_fromat_approve_date(pwf.approve_date::date)::character varying AS approve_date,
    case when pwf.approve_date is not null then true else false end::bool AS show_image
