...
CREATE OR REPLACE FUNCTION erp.get_report_po_approve(p_lin_id character varying, p_company_code character varying, p_doc_type character varying, p_doc_no character varying)
 RETURNS TABLE(work_flow_id integer, label_sign character varying, signature_image character varying, emp_name character varying, position_approve_desc character varying, approve_date character varying)
 LANGUAGE plpgsql
AS $function$
BEGIN
    RETURN QUERY
...
left join db_employee_name(p_lin_id) emp on emp.emp_id = pwf.approve_by_id 
