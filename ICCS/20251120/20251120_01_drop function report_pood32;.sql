...
left join db_employee_name(p_lin_id) deapprove on deapprove.emp_id = (select su.emp_id from su_user su 
																		where su.user_id = (select pwf.approve_by_id 
																							...
																							and pwf.doc_no = a.pr_no limit 1
																							)
																		limit 1)
...
AND EXISTS (
    ...
    WHERE pcm.pr_master_id = a.pr_master_id
      AND (
            (ppm.doc_type = 'PQ'  AND COALESCE(pct.tor_draft_committee, false) = true)
        OR  (ppm.doc_type <> 'PQ' AND COALESCE(pct.tor_draft_committee, false) = false)
      )
)
;
END;
