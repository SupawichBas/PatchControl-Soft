...
select a.pr_master_id::integer AS pr_master_id,
        	...
        	when (a.doc_type = 'PQ') then (SELECT 
					...
					FROM (
					    ...
					    LEFT JOIN po_committee_type_lang pctl 
					       ...
					       AND lower(pctl.language_code) = lower(p_lin_id)
					    WHERE pcm.pr_master_id = a.pr_master_id
		                and coalesce(pct.tor_draft_committee  ,false) = false --ไม่ใช่ TOR และราคากลาง
						) sub
					)
        ...
        case 
            ...
            when (select SUM(CASE WHEN COALESCE(pct.goods_receipt_committee, false) = false THEN 1 ELSE 0 END) = 0
        			...
        			where pcm.pr_master_id = a.pr_master_id
                    and coalesce(pct.tor_draft_committee  ,false) = false --ไม่ใช่ TOR และราคากลาง
    			 	)then null--มีแค่ ตรวจรับ แต่ไม่สน TOR
        	else (select pml.method_name from po_method_lang pml where pml.method_id = a.pr_method_id and lower(pml.language_code) = lower(p_lin_id))--วิธีซื้อจ้าง
        ...
        case 
        	when a.doc_type = 'PQ'
        		then concat(
						    ...
						    a.subject,' ',
						    'ประจำปีงบประมาณ พ.ศ. ',a.budget_year::varchar,' เพื่อให้เป็นไปตามพระราชบัญญัติการจัดซื้อจัดจ้างและการบริหารพัสดุภาครัฐ พ.ศ. 2560 และระเบียบกระทรวงการคลัง ว่าด้วยการจัดซื้อจัดจ้างและการบริหารพัสดุภาครัฐ พ.ศ. 2560 นั้น จึงขอแต่งตั้ง',
						    (SELECT 
							        pctl.committee_type_name AS names
							    FROM po_committee_master pcm 
							    JOIN po_committee_type pct 
						    ...
						    concat(case 
							    		when (select 'X'
					                        ...
					                        limit 1
					                        ) = 'X' then (select pml.method_name from po_method_lang pml where pml.method_id = a.pr_method_id and lower(pml.language_code) = lower(p_lin_id))
					                	else ''
					                end
...
AND EXISTS (
    SELECT 1
    FROM po_committee_master pcm
    LEFT JOIN po_committee_type pct 
           ON pct.committee_type_id = pcm.committee_type_id 
    JOIN po_pr_master ppm
           ON ppm.pr_master_id = a.pr_master_id
    WHERE pcm.pr_master_id = a.pr_master_id
      AND (
            (ppm.doc_type = 'PQ'  AND COALESCE(pct.tor_draft_committee, false) = true)--เป็น PQ สนใจแค่ TOR
        OR  (ppm.doc_type <> 'PQ' AND COALESCE(pct.tor_draft_committee, false) = false)--ไม่ใช้ PQ แล้วมีแค่ TOR จะไม่แสดง
      )
)
;
END;
