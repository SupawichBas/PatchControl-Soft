drop function report_pood32_sub;

CREATE OR REPLACE FUNCTION erp.report_pood32_sub(p_lin_id character varying, p_pr_master_id integer)
 RETURNS TABLE(pr_master_id integer, company_code character varying, doc_type character varying, doc_no character varying, ord_seq integer, committee_type character varying, committee_type_name_master character varying, com_code_mas integer, com_name_mas character varying, committee_id integer, com_name_det character varying, label_com_name_det character varying, label_com_name_mas character varying, remark character varying)
 LANGUAGE plpgsql
AS $function$
BEGIN
    RETURN QUERY
	...
	AND (
	    ( (SELECT ppm.doc_type FROM po_pr_master ppm WHERE ppm.pr_master_id = a.pr_master_id) = 'PQ' 
	        AND coalesce(pct.tor_draft_committee, false) = true )
	    OR
	    ( (SELECT ppm.doc_type FROM po_pr_master ppm WHERE ppm.pr_master_id = a.pr_master_id) <> 'PQ' 
	        AND coalesce(pct.tor_draft_committee, false) = false )
	)
