...
CREATE OR REPLACE FUNCTION erp.report_pood33(p_lin_id character varying, p_receive_master_id integer)
 RETURNS TABLE(receive_master_id integer, div_code character varying, div_name character varying, doc_type character varying, receive_no character varying, deliver_date character varying, status character varying, status_name character varying, receive_detail_id integer, seq integer, product_code character varying, product_name character varying, ums_name character varying, qty numeric, unit_price numeric, amt numeric
 , receive_set_id integer
 , seq_set integer
 , product_code_set character varying
 , product_name_set character varying
 , product_code_name_set character varying
 , ums_name_set character varying
 , QTY_set numeric
 , unit_price_set numeric
 , remark character varying, total_amt character varying, approve_image character varying, approve_date character varying, approve_name character varying, show_image boolean)
 LANGUAGE plpgsql
AS $function$
BEGIN
    RETURN QUERY
select a.receive_master_id::integer  as receive_master_id
		...
		,ROW_NUMBER() OVER (PARTITION BY b.receive_detail_id ORDER BY c.receive_set_id)::integer as seq_set
		,get_product_code(p_lin_id, c.product_type, c.product_id::integer, c.fa_type_id::integer, c.fa_mkind_id::integer)::character varying AS product_code_set
		,get_product_name(p_lin_id, c.product_type, c.product_id::integer, c.fa_type_id::integer, c.fa_mkind_id::integer)::character varying AS product_name_set
		,concat(get_product_code(p_lin_id, c.product_type, c.product_id::integer, c.fa_type_id::integer, c.fa_mkind_id::integer),' ',
		get_product_name(p_lin_id, c.product_type, c.product_id::integer, c.fa_type_id::integer, c.fa_mkind_id::integer))::character varying AS product_code_name_set
        ,get_ums_name(p_lin_id, c.product_type, c.ums_id::integer)::character varying AS ums_name_set
		,COALESCE(c.qty,0)::numeric as QTY_set
		,COALESCE(c.unit_price, 0)::numeric AS unit_price_set
		--sum
		...
		,concat('')::character varying as approve_date
		,(case when deapprove.emp_name is not null then concat('( ',deapprove.emp_name,' )')
				else ''
			end
			)::character varying as approve_name
		,case when a.status in ('A','E') then true else false end::bool as show_image
from po_receive_master a 
	left join db_employee_name(p_lin_id) deapprove on deapprove.emp_id = (select su.emp_id from su_user su where su.user_name = a.created_by limit 1)
left join po_receive_detail b on a.receive_master_id = b.receive_master_id 
left join po_receive_set c on b.receive_detail_id = c.receive_detail_id 
