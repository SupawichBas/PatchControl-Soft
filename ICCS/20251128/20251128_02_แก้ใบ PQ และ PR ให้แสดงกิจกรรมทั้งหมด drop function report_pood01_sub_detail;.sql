drop function report_pood01_sub_detail;

CREATE OR REPLACE FUNCTION erp.report_pood01_sub_detail(p_lin_id character varying, p_pr_master_id integer, p_atv_id integer)
 RETURNS TABLE(pr_master_id integer, company_code character varying, pr_no character varying, seq integer, product_code character varying, product_name character varying, qty numeric, ums_code character varying, ums_name character varying, unit_price numeric, amt numeric, detail_remark character varying, pr_detail_id integer, pr_set_id integer, set_seq integer, set_product_code character varying, set_product_name character varying, set_qty integer, set_ums character varying, set_ums_name character varying, set_unit_price numeric, set_amt numeric, set_remark character varying, forward_id character varying, bf_amt numeric, total_amt numeric, bel_amt numeric, pr_set_running_no integer, atv_id integer, pr_budget_id integer, pr_detail_running_no integer, group_atv_id integer)
 LANGUAGE plpgsql
AS $function$
BEGIN
    RETURN QUERY
