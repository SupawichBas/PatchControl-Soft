drop function report_pood02_sub;

CREATE OR REPLACE FUNCTION erp.report_pood02_sub(p_lin_id character varying, p_pr_master_id integer)
 RETURNS TABLE(pr_master_id integer, company_code character varying, doc_type character varying, doc_no character varying, ord_seq integer, committee_type character varying, committee_type_name_master character varying, com_code_mas integer, com_name_mas character varying, committee_id integer, com_name_det character varying, l_com_name_det character varying, l_com_name_mas character varying)
 LANGUAGE plpgsql
AS $function$
BEGIN
    RETURN QUERY
SELECT var.*
		,case when COUNT(*) OVER() > 1 then get_report_label_name('POOD02', 'com_name_mas', p_lin_id)
			else get_report_label_name('POOD02', 'com_name_mas2', p_lin_id) end::character varying AS L_com_name_mas
from (
