...
sum_amt AS (
    SELECT t.atv_id, SUM(t.amt)::numeric AS sum_amt_pr_detail
    FROM (
        SELECT DISTINCT var.atv_id, var.pr_detail_id, var.amt
        FROM var
    ) t
    GROUP BY t.atv_id
)
