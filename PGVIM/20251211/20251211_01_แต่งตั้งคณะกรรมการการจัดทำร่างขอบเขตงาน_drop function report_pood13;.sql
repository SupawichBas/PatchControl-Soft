...
--		    a.budget_year::varchar,
		    ' เพื่อให้เป็นไปตามพระราขบัญญัติการจัดซื้อจัดจ้าง และการบริหารพัสดุภาครัฐ พ.ศ. 2560 และระเบียบกระทรวงการคลังว่าด้วยการจัดซื้อจัดจ้างและการบริหารพัสดุภาครัฐ พ.ศ. 2560 นั้น จึงขอแต่งตั้งคณะกรรมการจัดทำร่างขอบเขตของงาน และกำหนดราคากลาง สำหรับดำเนินการ',
		    (select pcm.usage_for
						from po_committee_master pcm 
						join po_committee_type pct on pcm.committee_type_id = pct.committee_type_id 
						where pcm.pr_master_id = a.pr_master_id
		...
		concat('(1) จัดทำร่างขอบเขตของงานที่จะซื้อหรือจ้าง')::character varying as f1,
		concat('(2) กำหนดหลักเกณฑ์การพิจารณาคัดเลือกข้อเสนอ')::character varying as f2,
		concat('(3) กำหนดราคากลางของพัสดุที่จะซื้อหรือจ้าง')::character varying as f3,
        ...
        concat('สั่ง ณ วันที่ ',concat(to_char((select case when exists (select 1 from po_committee_master pcm
													...
													then (select pwf.approve_date from po_pr_ref ppr
															...
															and pwf.approve_by_id = (case when a.doc_type = 'PQ' then a.sign_id 
																						else a.approve_id 
																					end)
															and a.status in ('A','R')
                                                            order by pwf.seq desc
...
left join db_list_value_lang Monthlang  on Monthlang.group_code = 'Month' 
		and Monthlang.value = TO_CHAR((select case when exists (select 1 from po_committee_master pcm
													...
													then (select pwf.approve_date from po_pr_ref ppr
															...
															and a.status in ('A','R')
                                                            order by pwf.seq desc
                                                            limit 1) --ดึง approve_date ของใบ PR PQ แสดงตอนสถานะ A หรือ R
												end), 'FMMM')
