 DROP FUNCTION erp.report_porp07;

CREATE OR REPLACE FUNCTION erp.report_porp07(p_company_code character varying, p_lin_id character varying, p_s_po_date character varying, p_e_po_date character varying, p_s_div character varying, p_e_div character varying, p_budget_year character varying, p_user_id character varying, p_doc_type character varying)
 RETURNS TABLE(running_no integer, company_code character varying, receive_master_id integer, report_name character varying, pquarter character varying, year_receive_date character varying, month_receive_date character varying, ref_div_code character varying, ref_div_name character varying, div_code character varying, ap_member_id integer, ap_member character varying, sub_product_name character varying, total_amt numeric, receipt_date date, po_date date, format_po_date character varying, ref_no character varying, method_name character varying, status_name character varying, doc_type_name character varying)
 LANGUAGE plpgsql
AS $function$
BEGIN
    RETURN QUERY
...
from(
	select ppom.company_code::character varying as company_code
			...
			,concat(get_report_label_name('PORP07', 'ReportName', p_lin_id))::character varying as report_name
			,concat(
			    (CASE 
			...
			,coalesce(aml.full_name,coalesce(dpl.prefix_abbreviation, dpl.prefix_name), ' ', aml.first_name, ' ', aml.last_name)::character varying as ap_member
			,concat(case when (select count(ppod.pur_ord_detail_id)
								from po_pur_ord_detail ppod 
								where ppod.pur_ord_master_id = ppom.pur_ord_master_id) <> 1 then ppom.subject
					else (select get_product_name(p_lin_id, ppod.product_type, ppod.product_id::integer, ppod.fa_type_id::integer, ppod.fa_mkind_id::integer)
								from po_pur_ord_detail ppod 
								where ppod.pur_ord_master_id = ppom.pur_ord_master_id
								limit 1) end
					)::character varying as sub_product_name
			...
			,statusv.value_text::character varying as status_name
			,(select dlvl.value_text
				from db_list_value_lang dlvl
				where dlvl.group_code = 'PoDocTypePO'
				and lower(dlvl.language_code) = lower(p_lin_id)
				and dlvl.value = ppom.doc_type)::character varying as doc_type_name
	...
	left join db_list_value_lang statusv on statusv.group_code = (case when ppom.doc_type = 'PO' then 'PoStatusPO' else 'PoStatusCO' end)
			and statusv.value = ppom.status
			and lower(statusv.language_code) = lower(p_lin_id) 
	where 1=1
	and ppom.status not in ('C','D')
...
--	and to_char(extract(year from ppom.po_date) +543 ,'FM9999') = concat(p_budget_year)
	and ppom.doc_type = COALESCE(NULLIF(p_doc_type, '-'), ppom.doc_type)
	AND EXISTS (
			    SELECT 'x'
