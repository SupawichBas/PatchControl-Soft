...
--							,' ',TO_CHAR(a.deliver_date + INTERVAL '543 years', 'yyyy'))::character varying as deliver_date,
        	...
        	(select concat('ตามใบสั่งซื้อ/สัญญาที่ ',COALESCE(ppom.ref_no,'-'),' ลงวันที่ ',coalesce((select erp.po_format_date_thai(null, null, null, pwf.approve_date::date, p_lin_id) as max_approve_date
												from po_work_flow pwf
												where pwf.company_code = ppom.company_code
												and pwf.doc_type = ppom.doc_type 
												and pwf.doc_no = ppom.po_no 
												ORDER BY pwf.seq DESC
												LIMIT 1),'-') , ' ครบกำหนดส่งมอบ ภายในวันที่ ',
