DROP FUNCTION erp.report_pood03_sub_detail(varchar, int4);

CREATE OR REPLACE FUNCTION erp.report_pood03_sub_detail(p_lin_id character varying, p_pur_ord_master_id integer)
 RETURNS TABLE(pur_ord_master_id integer, company_code character varying, ap_member_id integer, ap_name character varying, ap_addr character varying, ap_tel character varying, ap_fax character varying, account_number character varying, ap_bank_name character varying, bank_name character varying, bank_branch_name character varying, ref_no character varying, po_date character varying, div_name_rep character varying, div_addr_rep character varying, tel_rep character varying, fax_rep character varying, pur_ord_detail_id integer, seq integer, product_code character varying, product_name character varying, qty numeric, ums_code character varying, ums_name character varying, unit_price numeric, gs_aft_disc numeric, pur_ord_set_id integer, set_seq integer, set_product_code character varying, set_product_name character varying, set_qty numeric, set_ums_code character varying, set_ums_name character varying, set_unit_price numeric, total_gs_aft_disc numeric, total_amt numeric, total_vat_amt numeric, baht_text character varying, lead_time integer, end_due_date_day character varying, end_due_date_month character varying, end_due_date_year character varying, rec_place character varying, gua_year integer, gua_month integer, fine_rate numeric, fine_ums character varying, fine_type character varying, ref_qo_no character varying, recipient_personal_image character varying, personal_name character varying, position_type character varying, sign_image character varying, sign_name character varying, sign_posi character varying, approve_posi character varying, status_show_image boolean, fh character varying, f0 character varying, f1 character varying, f2 character varying, f3 character varying, f4 character varying, f5 character varying, approve_date_sign_id character varying, remark_detail character varying, remark_set character varying)
 LANGUAGE plpgsql
AS $function$
BEGIN
    RETURN QUERY
	select a.pur_ord_master_id::integer as pur_ord_master_id,
			...
			coalesce(aml.full_name, coalesce(aml2.prefix_abbreviation, aml2.prefix_name), ' ', aml.first_name, ' ', aml.last_name)::character varying as ap_name,
			...
			case 
				when a.product_type = '2' then case
													when length(b.remark) = 0 or b.remark is null then get_product_name(p_lin_id, b.product_type, b.product_id::integer, b.fa_type_id::integer, b.fa_mkind_id::integer)
													else b.remark
												end
			...
			--end detail
			--set
			c.pur_ord_set_id::integer as pur_ord_set_id,
			...
			get_ums_code(p_lin_id, c.product_type, c.ums_id::integer)::character varying AS set_ums_code,
			...
			--end set
			COALESCE(a.total_amt, 0) - coalesce(a.total_vat_amt,0)::numeric AS total_gs_aft_disc,
			...
			concat('')::character varying AS approve_date_sign_id,
			case 
				when a.product_type = '2' then null
				else (case when length(b.remark) = 0 or b.remark is null then '' else concat('',b.remark) end)
			end::character varying AS remark_detail,
			(case when length(c.remark) = 0 or c.remark is null then '' else concat('',c.remark) end)::character varying AS remark_set
	...
	left join po_pur_ord_set c on c.pur_ord_detail_id = b.pur_ord_detail_id 
