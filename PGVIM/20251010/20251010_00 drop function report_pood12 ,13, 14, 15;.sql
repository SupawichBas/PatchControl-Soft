...
select a.pr_master_id::integer AS pr_master_id,
        ...
        concat('เรื่อง ','แต่งตั้งคณะกรรมการตรวจรับพัสดุ')::character varying AS report_name,
        concat('สำหรับ',(select pcm.usage_for
        ...
        concat('ประจำปีงบประมาณ พ.ศ. ',a.budget_year)::character varying AS budget_year_header,
        concat('ที่ ',a.ref_no)::character varying AS ref_no,
        concat('อำนาจและหน้าที่')::character varying AS field_summary1,
        concat('ทำการตรวจรับพัสดุให้เป็นไปตามเงื่อนไขของสัญญาหรือข้อตกลงนั้น')::character varying AS field_summary2,
        ...
        concat(
		    ...
		    ' เพื่อให้เป็นไปตามหลักเกณฑ์ของระเบียบการทรวงการคลังว่าด้วยการจัดซื้อจัดจ้างและการบริหารพัสดุภาครัฐ พ.ศ. 2560 ข้อ 25 (5) ประกอบข้อ 175 ',
		    (select l.company_name from db_company_lang l where l.company_code = a.company_code and lower(l.language_code) = lower(p_lin_id)),
		    ' จึงเห็นสมควร',
        ...
        concat('สั่ง ณ วันที่ ',concat(to_char((select pwf.approve_date from po_work_flow pwf where work_flow_id = a.pr_master_id), 'FMDD') ,' ' ,
							Monthlang.value_text ,(case when Monthlang.value_text is not null then ' พ.ศ. ' else'' end),
							to_char(extract(year from (select pwf.approve_date from po_work_flow pwf where work_flow_id = a.pr_master_id)) +543 ,'FM9999')))::character varying as pr_req_date,
...
left join db_list_value_lang Monthlang  on Monthlang.group_code = 'Month' 
		and Monthlang.value = TO_CHAR((select pwf.approve_date from po_work_flow pwf where work_flow_id = a.pr_master_id), 'FMMM')
...
select a.pr_master_id::integer AS pr_master_id,
        concat('คำสั่ง ', (select l.company_name from db_company_lang l where l.company_code = a.company_code and lower(l.language_code) = lower(p_lin_id)))::character varying AS company_name,
        concat('ที่ ',a.ref_no)::character varying AS ref_no,
        concat('เรื่อง ','แต่งตั้งคณะกรรมการการจัดทำร่างขอบเขตงาน และกำหนดราคากลาง')::character varying AS report_name,
        concat('สำหรับ',(select pcm.usage_for
						from po_committee_master pcm 
						join po_committee_type pct on pcm.committee_type_id = pct.committee_type_id 
						where pcm.pr_master_id = a.pr_master_id
        ...
        concat(
		    ...
		    ' ประจำปีงบประมาณ พ.ศ. ',
		    a.budget_year::varchar,
		    ' เพื่อให้เป็นไปตามพระราขบัญญัติการจัดซื้อจัดจ้างและการบริหารพัสดุภาครัฐ พ.ศ. 2560 และระเบียบกระทรวงการคลังว่าด้วยการจัดซื้อจัดจ้างและการบริหารพัสดุภาครัฐ พ.ศ. 2560 นั้น จึงขอแต่งตั้งคณะกรรมการจัดทำร่างขอบเขตของงาน และกำหนดราคากลาง สำหรับดำเนินการ',
		    a.subject,
		    ' ประจำปีงบประมาณ พ.ศ. ',
		    a.budget_year::varchar,
		    ' ดังผู้มีรายนามดังต่อไปนี้'
		...
		concat('(1) จัดทำร่างขอบเขตของงานที่จะซื้อ')::character varying as f1,
		concat('(2) กำหนดหลักเกณฑ์การพิจารณาคัดเลือกข้อเสนอ')::character varying as f2,
		concat('(3) กำหนดราคากลางของพัสดุที่จะซื้อ')::character varying as f3,
		concat('(4) ให้คณะกรรมการฯ ดำเนินการตามข้อ 1 และข้อ 2 ให้แล้วเสร็จภายใน 15 วัน นับถัดจากวันที่มีคำสั่งแต่งตั้ง')::character varying as f4,
        concat('สั่ง ณ วันที่ ',concat(to_char((select pwf.approve_date from po_work_flow pwf where work_flow_id = a.pr_master_id), 'FMDD') ,' ' ,
							Monthlang.value_text ,(case when Monthlang.value_text is not null then ' พ.ศ. ' else'' end),
							to_char(extract(year from (select pwf.approve_date from po_work_flow pwf where work_flow_id = a.pr_master_id)) +543 ,'FM9999')))::character varying as pr_req_date,
        case when deapprove.personal_id is not null and deapprove.signature_image_id is not null
...
CREATE OR REPLACE FUNCTION erp.report_pood14(p_lin_id character varying, p_pr_master_id integer)
 RETURNS TABLE(pr_master_id integer, company_name character varying, ref_no character varying, report_name character varying, subject_header character varying, budget_year_header character varying, field_detail character varying, f0 character varying, f1 character varying, pr_req_date character varying, recipient_sign_image character varying, sign_name character varying, status_show_image boolean, label_sign_approve character varying)
 LANGUAGE plpgsql
AS $function$
BEGIN
    RETURN QUERY
select a.pr_master_id::integer AS pr_master_id,
        concat('คำสั่ง ', (select l.company_name from db_company_lang l where l.company_code = a.company_code and lower(l.language_code) = lower(p_lin_id)))::character varying AS company_name,
        concat('ที่ ',a.ref_no)::character varying AS ref_no,
        concat('เรื่อง ','แต่งตั้งคณะกรรมการ การจัดซื้อจัดจ้าง')::character varying AS report_name,
        concat('สำหรับ',(select pcm.usage_for
						from po_committee_master pcm 
						join po_committee_type pct on pcm.committee_type_id = pct.committee_type_id 
						where pcm.pr_master_id = a.pr_master_id
        ...
        concat(
		    ...
		    (select l.company_name from db_company_lang l where l.company_code = a.company_code and lower(l.language_code) = lower(p_lin_id)),
		    ' จึงเห็นสมควร',
		    'แต่งตั้งคณะกรรมการ การจัดซื้อจัดจ้าง',
		    ' สำหรับ',
		    a.subject,
		    ' ประจำปีงบประมาณ พ.ศ. ',
		    a.budget_year::varchar,
		...
		concat('อำนาจและหน้าที่')::character varying as f0,
		concat('ทำการ การจัดซื้อจัดจ้าง ให้เป็นไปตามเงื่อนไขของสัญญาหรือข้อตกลงนั้น')::character varying as f1,
        concat('สั่ง ณ วันที่ ',concat(to_char((select pwf.approve_date from po_work_flow pwf where work_flow_id = a.pr_master_id), 'FMDD') ,' ' ,
							Monthlang.value_text ,(case when Monthlang.value_text is not null then ' พ.ศ. ' else'' end),
							to_char(extract(year from (select pwf.approve_date from po_work_flow pwf where work_flow_id = a.pr_master_id)) +543 ,'FM9999')))::character varying as pr_req_date,
        case when deapprove.personal_id is not null and deapprove.signature_image_id is not null
