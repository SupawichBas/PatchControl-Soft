...
--							,' ',TO_CHAR(a.deliver_date + INTERVAL '543 years', 'yyyy'))::character varying as deliver_date,
        	...
        	ROW_NUMBER() OVER (PARTITION BY a.receive_master_id, b.receive_detail_id ORDER BY c.receive_set_id)::integer AS receive_set_running_no
...
--            			
    ...
    CTE.*,
    DENSE_RANK() OVER (ORDER BY CTE.receive_detail_id)::integer AS receive_detail_running_no,
