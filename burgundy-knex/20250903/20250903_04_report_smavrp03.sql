...
begin
	...
	select (concat(get_company_code_name(p_lin_id, p_company_code),' / ',
		...
		coalesce (dbo.get_division_name(p_lin_id, p_company_code, p_branch_code, p_division_code), dbo.get_report_label_name('SMAVRP03', 'ALL', p_lin_id))::varchar  as P_Division,
		coalesce (dbo.get_employee_name(p_lin_id, p_company_code, p_branch_code, p_requester_user_code), dbo.get_report_label_name('SMAVRP03', 'ALL', p_lin_id))::varchar  as P_Emp,
		coalesce (dbo.get_document_sub_type(p_lin_id, p_type_of_advance), dbo.get_report_label_name('SMAVRP03', 'ALL', p_lin_id))::varchar  as P_Document_Sub_Type,
		coalesce (dbo.get_status_name(p_lin_id, p_company_code, p_status), dbo.get_report_label_name('SMAVRP03', 'ALL', p_lin_id))::varchar  as p_status_Name,
...
--		join av_document_item adi on adi.av_document_id = ad.av_document_id 
	where wf.company_code = p_company_code--'SS'
		...
		and	 CAST(ad.av_start_date AS DATE)
