drop function report_smavrp03_excel;

CREATE OR REPLACE FUNCTION dbo.report_smavrp03_excel(user_company_code character varying, p_lin_id character varying, p_company_code character varying, p_branch_code character varying, p_division_code character varying, p_requester_user_code character varying, p_s_date character varying, p_e_date character varying, p_type_of_advance character varying, user_login integer, p_status character varying)
 RETURNS TABLE(h_company character varying, p_companybranch character varying, p_division character varying, p_emp character varying, p_document_sub_type character varying, p_status_name character varying, division_code character varying, division_name character varying, av_document_id integer, av_document_item_id integer, running_number_av_document integer, av_document_id_count integer, document_no character varying, av_start_date character varying, av_due_date character varying, requester_user_code character varying, full_name character varying, subject character varying, curr_amount numeric, currency_code character varying, local_amount_of_approve numeric, dueday integer, trackingtimes integer, status_value character varying, status_name character varying, work_flow_id integer, work_flow_type_code character varying, local_amount_of_approve_ad numeric, work_flow_id_ex integer, work_flow_type_code_ex character varying, document_no_ex character varying, document_date_ex character varying, total_expense_amount numeric, banktocom numeric, navigatereport character varying)
 LANGUAGE plpgsql
AS $function$ 
...
begin
	...
	select (concat(get_company_code_name(p_lin_id, p_company_code),' / ',
		...
		coalesce(adi.curr_amount,0)::numeric as curr_amount,
		...
		wfex.work_flow_id::integer as work_flow_id_ex,
		wfex.work_flow_type_code::varchar as work_flow_type_code_ex,
		wfex.document_no::varchar as document_no_ex,
		to_char(wfex.document_date , 'dd/MM/yyyy')::varchar as document_date_ex ,
	...
	from work_flow wf 
		...
		left join fn_expense_reference fer on fer.av_document_id = ad.av_document_id
			left join work_flow wfex on fer.work_flow_id = wfex.work_flow_id 
			left join fn_expense_document fed on fed.work_flow_id = wfex.work_flow_id 
