...
BEGIN
    ...
    SELECT 
        ...
        po_get_full_address(am.house_no, am.village, am.building, am.moo, am.soi, am.road, am.sub_district_id, am.district_id, am.province_id, am.postal_code_id, 'PODT07', upper(p_lin_id))::character varying AS parameter3,
        concat(
        ...
        a.div_name_rep::character varying AS parameter6,
        ...
        concat('กำหนดส่งมอบภายใน',' ', coalesce(a.lead_time,0), ' ', 'วันทำการ นับถัดจากวันที่ผู้ขายได้รับใบสั่งซื้อ')::character varying AS footer1,
        concat('ครบกำหนดส่งมอบวันที่',' ', coalesce(TO_CHAR(a.end_due_date  + INTERVAL '543 years', 'fmDD'),'-'), ' ', coalesce(Monthlang.value_text,'-'),' พ.ศ', ' ',coalesce(TO_CHAR(a.end_due_date + INTERVAL '543 years', 'yyyy'),'-'))::character varying AS footer2,
        concat('สถานที่ส่งมอบ', ' ', coalesce(a.rec_place,'-'))::character varying AS footer3,
        concat('ระยะเวลารับประกัน', ' ', coalesce(a.gua_year::character varying,'-'), ' ', 'ปี', ' ', coalesce(a.gua_month::character varying,'-'), ' ', 'เดือน')::character varying AS footer4,
        concat(
        ...
        )::character varying AS footer5,
        concat('ส่วนราชการสงวนสิทธิ์ที่จะไม่รับมอบถ้าหากปรากฏว่าสินค้านั้นมีลักษณะไม่ตรงตามรายการที่ระบุไว้ในใบสั่งซื้อ กรณีนี้ผู้ขายจะต้องดำเนินการเปลี่ยนใหม่ให้ถูกต้องตามใบสั่งซื้อทุกประการ')::character varying AS footer6,
        concat('การจ้างช่วง ผู้รับจ้างจะต้องไม่เอางานทั้งหมดหรือแต่บางส่วนไปจ้างช่วงอีกทอดหนึ่ง เว้นแต่การจ้างช่วงงานแต่บางส่วนที่ได้รับอนุญาตเป็นหนังสือจากผู้ว่าจ้างแล้ว การที่ผู้ว่าจ้างได้อนุญาตให้จ้างช่วงงานแต่บางส่วนดังกล่าวนั้น ไม่เป็นเหตุให้ผู้รับจ้างหลุดพ้นจากความรับผิดหรือพันธะหน้าที่และผู้รับจ้างจะยังคงต้องรับผิดในความผิดและความประมาทเลินเล่อ ของผู้รับจ้างช่วงหรือของตัวแทน หรือลูกจ้างของผู้รับจ้างช่วงนั้นทุกประการ กรณีผู้รับจ้างไปจ้างช่วงงานแต่บางส่วน โดยฝ่าฝืนความในวรรคหนึ่ง ผู้รับจ้างต้องชำระค่าปรับ ให้แก่ผู้ว่าจ้างเป็นจำนวนเงินในอัตราร้อยละ 10 (สิบ) ของวงเงินที่จ้างช่วง ทั้งนี้ ไม่ตัดสิทธิผู้ว่าจ้างในการบอกเลิกสัญญา')::character varying AS footer7,
        concat('การประเมินผลการปฏิบัติงานของผู้ประกอบการหน่วยงานของรัฐสามารถนำผลการปฏิบัติงานแล้วเสร็จตามสัญญา หรือข้อตกลงของคู่สัญญาเพื่อนำมาประเมินผลการปฏิบัติงานของผู้ประกอบการ')::character varying AS footer8,
