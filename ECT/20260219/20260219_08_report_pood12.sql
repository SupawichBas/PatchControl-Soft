-- DROP FUNCTION erp.report_pood12(varchar, int4);

CREATE OR REPLACE FUNCTION erp.report_pood12(p_lin_id character varying, p_guaranty_master_id integer)
 RETURNS TABLE(guaranty_master_id integer, guaranty_no character varying, guaranty_date character varying, ref_doc_no character varying, cp_date character varying, po_no character varying, start_end_gua character varying, receipt_code character varying, method_name character varying, ref_no character varying, cp_subject character varying, ap_code_name character varying, cp_period text, recieve_emp character varying, collateral_holder character varying, company_name character varying)
 LANGUAGE plpgsql
AS $function$
BEGIN
    RETURN QUERY
	select 		
		...
		(		case 
			when a.ref_doc_type = 'PC' then (
				...
				select case when az.po_date is not null then 
					case p_lin_id when 'TH' then 
