...
SELECT 
    ...
    concat(a.contract_code_egp)::character varying AS parameter3,--ที่
    ...
    concat('ขออนุมัติเบิกจ่ายเงิน',ppm.subject)::character varying AS parameter5,--เรื่อง
    ...
    concat(
        ...
        concat(
            ...
            ' ได้ส่งมอบ', ppm.subject,' จำนวน ',
			(select to_char(count(prd.receive_detail_id), 'FM999,999,999') from po_receive_detail prd where prd.receive_master_id = a.receive_master_id),
			' รายการ ',concat('ตาม ',(select dlvl.value_text 
            ...
            'และ',a.attn_to,' ได้รับทราบรายงานผลการตรวจรับแล้ว ตามเอกสารที่แนบมาพร้อมนี้'
        )::character varying AS parameter8,--ข้อเท็จจริง
    concat('เจ้าหน้าที่ได้ตรวจสอบแล้ว เห็นควรอนุมัติให้เบิกจ่ายเงินค่า', ppm.subject,' เป็นจำนวนเงินทั้งสิ้น ',
            TO_CHAR(COALESCE(a.total_amt, 0)::numeric, 'FM99,999,999.00'), ' บาท', ' (', erp.po_baht_text(a.total_amt), ') ',
            'ให้แก่ ',concat(aml2.prefix_abbreviation, ' ',aml.first_name,' ',aml.last_name)
        )::character varying AS parameter9,--ข้อพิจารณา
    concat('จึงเรียนมาเพื่อโปรดอนุมัติ')::character varying AS parameter10--ข้อเสนอ
