...
BEGIN
    ...
    SELECT 
	    ...
	    concat((select concat(l.company_name,'ประจำจังหวัด',dpl.province_name ) from db_company_lang l
            												left join db_province_lang dpl on dpl.province_id = l.province_id
            														and lower(l.language_code) = lower(dpl.language_code)
            												where l.company_code = ppm.company_code
            												and lower(l.language_code) = lower(p_lin_id)))::character varying AS parameter1,
	    concat(ppm.ref_no)::character varying AS parameter2,
	    ...
	    concat('ตามที่ ',(select concat(l.company_name,'ประจำจังหวัด',dpl.province_name ) from db_company_lang l
            												left join db_province_lang dpl on dpl.province_id = l.province_id
            														and lower(l.language_code) = lower(dpl.language_code)
