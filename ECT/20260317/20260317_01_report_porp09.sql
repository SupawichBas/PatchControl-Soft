-- DROP FUNCTION erp.report_porp09;
...
CREATE OR REPLACE FUNCTION erp.report_porp09(p_quarter character varying, p_company_code character varying, p_lin_id character varying, p_quarter1 character varying, p_quarter2 character varying, p_s_div character varying, p_e_div character varying, p_budget_year character varying, p_user_id character varying)
 RETURNS TABLE(running_no integer, company_code character varying, pr_master_id integer, pquarter character varying, year_pr_date character varying, month_pr_date character varying, ref_div_code character varying, ref_div_name character varying, div_code character varying, ap_member_id integer, ap_member character varying, sub_product_name character varying, total_amt numeric, pr_date date, format_pr_date character varying, ref_no character varying)
 LANGUAGE plpgsql
AS $function$
BEGIN
    RETURN QUERY
...
--	and to_char(extract(year from ppm.pr_date) +543 ,'FM9999') = p_budget_year
	...
	AND (
		    ( coalesce(ppm.is_special_procurement,false) is false AND ppom.pur_ord_master_id IS NOT null and ppm.status in ('A','E','R'))--เป็นการจัดซื้อปกติ
		    OR 
		    ( coalesce(ppm.is_special_procurement,false) is true AND ppm.status in ('A','E','R'))--เป็นการจัดซื้้อแบบ ว.119
		)
