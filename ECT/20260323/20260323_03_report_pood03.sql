DROP FUNCTION erp.report_pood03;
...
BEGIN
	...
	SELECT 
	    ...
	    concat(a.phone_number)::character varying AS parameter2,
	    ...
	    concat('รายงานผลพิจารณาและขออนุมัติจัดซื้อจัดจ้าง ',a.subject)::character varying AS parameter5,
	    concat(a.attn_to)::character varying AS parameter6,
	    concat('ตามที่ เลขาธิการคณะกรรมการเลือกตั้ง ได้อนุมัติให้',a.subject,' จำนวน ',
				(select to_char(count(ppd.pr_detail_id), 'FM999,999,999') from po_pr_detail ppd where ppd.pr_master_id = a.pr_master_id),
				' รายการ เพื่อ',a.purpose,' โดย',pml.method_name,' ภายในวงเงินงบประมาณ ',
	    ...
	    concat('รายงานผลการพิจารณางาน',a.subject,' จำนวน ',
				(select to_char(count(ppd.pr_detail_id), 'FM999,999,999') from po_pr_detail ppd where ppd.pr_master_id = a.pr_master_id),
				' รายการ',' ผู้เสนอราคาที่ได้รับ การคัดเลือก ได้แก่ ',concat(aml2.prefix_abbreviation, ' ',aml.first_name,' ',aml.last_name),
	    		'ราคาที่เสนอเป็นจำนวนเงิน ',
               to_char(a.gs_bef_disc, 'FM999,999,999.00'),
               ' บาท ',
               '(', erp.po_baht_text(a.gs_bef_disc), ')',(
	               case when a.disc_perc is not null or a.disc_baht is not null
	               		then concat(' และเจ้าหน้าที่ ได้ต่อรองราคากับ ',concat(aml2.prefix_abbreviation, ' ',aml.first_name,' ',aml.last_name),
																				' แล้ว ','ทาง',concat(aml2.prefix_abbreviation, ' ',aml.first_name,' ',aml.last_name),
																				' ยินดีลดราคาเหลือเพียง ',
																				to_char(a.gs_aft_disc, 'FM999,999,999.00'),
																               ' บาท ',
																               '(', erp.po_baht_text(a.gs_aft_disc), ')',
																               ' จึงได้ตกลงซื้อกับ ',concat(aml2.prefix_abbreviation, ' ',aml.first_name,' ',aml.last_name),
																	    		'เป็นจำนวนเงิน ',
																				to_char(a.total_amt, 'FM999,999,999.00'),
																               ' บาท ',
																               '(', erp.po_baht_text(a.total_amt), ')')
	               		else concat(' และเจ้าหน้าที่ ได้ต่อรองราคากับ ',concat(aml2.prefix_abbreviation, ' ',aml.first_name,' ',aml.last_name),
																				' แล้ว ได้รับแจ้งว่าเป็นราคาต่ำสุดไม่สามารถลดราคาลงได้ ',' จึงได้ตกลงซื้อกับ ',concat(aml2.prefix_abbreviation, ' ',aml.first_name,' ',aml.last_name),
																	    		'เป็นจำนวนเงิน ',
																				to_char(a.total_amt, 'FM999,999,999.00'),
																               ' บาท ',
																               '(', erp.po_baht_text(a.total_amt), ')')
	               end))::character varying AS parameterdetail2,
	    concat('จึงเรียนมาเพื่อโปรดพิจารณา หากเห็นชอบขอได้โปรดอนุมัติให้สั่งซื้อจากผู้เสนอราคาที่ได้รับการคัดเลือกและลงนามประกาศผลผู้ได้รับการคัดเลือกตามมาตรา 66 แห่งพระราชบัญญัติการจัดซื้อจัดจ้างและการบริหารพัสดุภาครัฐ พ.ศ. 2560 ด้วย')::character varying AS parameterdetail3,
